class ProblemsController < ApplicationController
	...
	def cases
		...
		# REPORT ANALYSIS SECTION #
			...
			# Start new round of questions with the same diagnoses as previously
			...
			if @learnerinfo.current_index < 10
				...
				if params[:viewreport]
					@studentreport = StudentReport.where(:id => params[:viewreport]).first
					@view_exp = ExpertReport.where(:id => @studentreport.expert_report_id).first
					@expert_sentences = @view_exp.report_text.split(".")
					@student_sentences = @studentreport.report_text.split(".")
				end

				@skipped_dx = Array.new
				@attempted_dx = Array.new

				@learnerinfo.report_array.each do |l|
					@skipped_dx << l
				end
				...
				@learnerinfo.answers_array.each do |a|
					...
					@eid = @l.expert_report.end_dx_id
					@skipped_dx.delete_at(@skipped_dx.index(@eid))
					@attempted_dx << @eid
				...
				@skippedlist = EndDx.where("id IN (?)", @skipped_dx)
